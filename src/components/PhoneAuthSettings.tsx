...
export const PhoneAuthSettings: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl p-4 sm:p-8 shadow-sm border border-slate-200"
    >
      {/* Header */}
      <div className="flex flex-col gap-6 mb-8 pb-6 border-b border-slate-100">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          ...
          {/* Tab Switcher */}
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200 w-full sm:w-auto overflow-x-auto no-scrollbar">
            <button
              type="button"
              onClick={() => setActiveTab('config')}
              className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-xl text-[10px] sm:text-xs font-bold transition-all flex items-center justify-center gap-2 whitespace-nowrap ${
                activeTab === 'config'
      ...
      {activeTab === 'config' ? (
        <div className="space-y-8">
          ...
          <div className="p-4 sm:p-5 bg-gradient-to-r from-emerald-900 to-indigo-950 text-white rounded-2xl shadow-md flex flex-col gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2 font-black text-[10px] sm:text-sm text-emerald-300 uppercase tracking-wider">
                <ArrowRightLeft size={16} className="shrink-0" />
                <span className="break-words">WhatsApp Primary + Automatic SMS Fallback Strategy</span>
              </div>
              <p className="text-[10px] sm:text-xs text-slate-200 leading-relaxed max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 font-bold text-[9px] sm:text-xs rounded-full whitespace-nowrap">
                WhatsApp Active
              </span>
              <span className="px-3 py-1 bg-indigo-500/20 border border-indigo-400/40 text-indigo-300 font-bold text-[9px] sm:text-xs rounded-full whitespace-nowrap">
                SMS Fallback Ready
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-4 sm:p-6 bg-slate-50 rounded-2xl border border-slate-200/80">
            <div className="flex items-center sm:items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] sm:text-xs text-slate-500 mt-1 leading-tight">
                  New users must register with a mobile number and verify OTP.
                </p>
              </div>
              ...
              {[
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-bold text-sm text-slate-900 break-words">{provider.name}</span>
                    {(config.whatsappProvider || 'simulation') === provider.id && (
                      <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                    )}
                  </div>
                  <span className="inline-block mt-2 text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-md bg-slate-100 text-slate-600">
                    {provider.badge}
                  </span>
                </button>
              ))}
          ...
          <div className="p-4 sm:p-6 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-4">
            <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl font-mono text-sm focus:border-indigo-600 focus:ring-0"
              />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  className="w-full px-5 py-3 bg-emerald-600 text-white rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider hover:bg-emerald-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                >
                  {isTestingWhatsapp ? (
                ...
                <button
                  ...
                  className="w-full px-5 py-3 bg-indigo-600 text-white rounded-xl font-bold text-[10px] sm:text-xs uppercase tracking-wider hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                >
                  {isTestingSms ? (
                    <RefreshCw className="w-4 h-4 animate-spin" />
                  ) : (
          ...
          <div className="flex justify-center sm:justify-end pt-4">
            <button
              ...
              onClick={handleSave}
              disabled={isSaving}
              className="w-full sm:w-auto px-8 py-4 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-wider text-xs hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 flex items-center justify-center gap-2"
            >
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex-1 min-w-[150px] flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-semibold">
                <Filter size={14} className="text-slate-500 shrink-0" />
                <select
                  value={logFilterChannel}
                  onChange={(e) => setLogFilterChannel(e.target.value as any)}
                  className="bg-transparent border-none focus:ring-0 text-slate-800 font-bold w-full"
                >
